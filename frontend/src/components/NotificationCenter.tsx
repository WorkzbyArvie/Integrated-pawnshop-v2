...
export function NotificationCenter({ userId }: NotificationCenterProps) {
  ...
  const getIcon = (type: NotificationType) => {
    switch (type) {
      ...
      default:
        return <Info className="w-4 h-4 text-[#8A8279]" />;
    }
  };
      ...
      {open && (
        <div className="absolute left-full top-12 ml-6 w-96 bg-[#14141B] rounded-2xl shadow-2xl border border-[rgba(201,160,92,0.12)] z-[60] overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-[rgba(201,160,92,0.08)]">
            <h3 className="font-bold text-[#F5F0E8] text-sm">Notifications</h3>
            {unreadCount > 0 && (
          ...
          <div className="max-h-[400px] overflow-y-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center py-10 text-[#8A8279]">
              ...
              notifications.map((n) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm leading-snug ${n.status !== 'READ' ? 'font-semibold text-[#F5F0E8]' : 'text-[#B8B0A4]'}`}>
                      {n.title}
                    </p>
                    <p className="text-xs text-[#8A8279] mt-0.5 line-clamp-2">{n.body}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] text-[#8A8279] flex items-center gap-1">
