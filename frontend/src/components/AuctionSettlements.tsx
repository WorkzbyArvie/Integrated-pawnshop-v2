    ...
    const result = await Swal.fire({
      ...
      html: `
        ...
        <textarea id="release-notes" placeholder="Release notes (optional)"
          ...
          background:rgba(255,255,255,0.04);color:#F5F0E8;font-family:inherit;resize:vertical;"
          rows="3"></textarea>
      `,
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-black text-[#F5F0E8] uppercase tracking-tight">
            Auction Settlements
          </h2>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2 rounded-xl bg-[#1C1C26] border border-[rgba(201,160,92,0.08)] text-[#F5F0E8] text-sm placeholder-[#8A8279] focus:outline-none focus:border-[#C9A05C] w-64"
          />
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-[#8A8279]">Loading settlements...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="border-2 border-dashed border-[rgba(201,160,92,0.08)] rounded-[2rem] flex flex-col items-center justify-center py-24 px-8 text-center">
          <ShieldCheck className="w-8 h-8 text-[#8A8279] mb-4" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[rgba(201,160,92,0.08)]">
                <th className="px-4 py-3 text-left text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Item</th>
                <th className="px-4 py-3 text-left text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Winner</th>
                <th className="px-4 py-3 text-right text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Winning Bid</th>
                <th className="px-4 py-3 text-center text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Status</th>
                <th className="px-4 py-3 text-center text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Contract</th>
                <th className="px-4 py-3 text-center text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((item) => {
                ...
                return (
                  <tr key={item.id} className="border-b border-[rgba(201,160,92,0.04)] hover:bg-[rgba(201,160,92,0.02)] transition-colors">
                    <td className="px-4 py-4">
                      <p className="font-bold text-[#F5F0E8]">{item.listing.title}</p>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        <UserCheck className="w-4 h-4 text-[#8A8279]" />
                        <span className="font-bold text-[#F5F0E8]">{item.winnerFullName}</span>
                      </div>
                      <p className="text-[10px] text-[#8A8279]">{item.winnerPhone}</p>
                    </td>
                    <td className="px-4 py-4 text-right font-bold text-[#F5F0E8]">
                    ...
                    <td className="px-4 py-4 text-center">
                      {item.contractSignedAt ? (
                        ...
                        <span className="text-[10px] text-[#8A8279]">Not signed</span>
                      )}
                    </td>
                    <td className="px-4 py-4 text-center">
                      {item.status === 'RELEASED' ? (
                        ...
                        <span className="text-[10px] text-[#8A8279]">Awaiting payment</span>
                      )}
                    </td>
                  </tr>
                );
