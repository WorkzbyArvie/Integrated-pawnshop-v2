  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 bg-[#14141B] rounded-[3rem] border border-dashed border-[rgba(201,160,92,0.12)]">
        ...
        <p className="text-[#8A8279] font-black uppercase tracking-widest text-[10px]">Syncing Client Ledger...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="relative w-full md:w-[500px]">
          ...
          <input 
            ...
            className="w-full pl-14 pr-6 py-4 bg-[#14141B] border border-[rgba(201,160,92,0.08)] shadow-sm rounded-2xl text-sm font-bold placeholder:text-[#8A8279] focus:ring-2 focus:ring-[#C9A05C]/20 transition-all outline-none"
            value={searchTerm}
        ...
        <div className="flex items-center gap-3">
          <div className="bg-[#14141B] px-4 py-2 rounded-xl border border-[rgba(201,160,92,0.08)] shadow-sm">
            <p className="text-[8px] font-black text-[#8A8279] uppercase tracking-tighter">Total Records</p>
                ...
                <div className="absolute right-0 top-full mt-2 z-50 w-48 bg-[#1C1C26] border border-[rgba(201,160,92,0.12)] rounded-2xl p-1.5 shadow-2xl">
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors ${
                      tierFilter === 'all' ? 'bg-[#C9A05C]/20 text-[#C9A05C]' : 'text-[#B8B0A4] hover:bg-[#14141B]'
                    }`}
                  >
                    All Tiers
                  </button>
                  {TIERS.map((tier) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors ${
                        tierFilter === tier ? 'bg-[#C9A05C]/20 text-[#C9A05C]' : 'text-[#B8B0A4] hover:bg-[#14141B]'
                      }`}
                    >
                      {normalizeTier(tier)}
                    </button>
      ...
      <div className="bg-[#14141B] rounded-[2.5rem] border border-[rgba(201,160,92,0.08)] shadow-sm overflow-hidden">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-[#1C1C26]/50">
                <th className="px-8 py-5 text-[10px] font-black text-[#8A8279] uppercase tracking-widest">Client Identity</th>
                <th className="px-8 py-5 text-[10px] font-black text-[#8A8279] uppercase tracking-widest">Contact Info</th>
                <th className="px-8 py-5 text-[10px] font-black text-[#8A8279] uppercase tracking-widest">Status</th>
              </tr>
            </thead>
                ...
                >
                  ...
                  <td className="px-8 py-6">
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2 text-xs font-bold text-[#8A8279]">
                        <Phone className="w-3.5 h-3.5 text-[#C9A05C]" /> {customer.contact_number || 'N/A'}
                      </div>
                      <div className="flex items-center gap-2 text-[11px] font-medium text-[#8A8279]">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-[10px] font-black text-[#F5F0E8] uppercase">Active</p>
                        <p className="text-[9px] font-bold text-[#8A8279] uppercase tracking-tighter">ID: {customer.id.slice(0, 8)}</p>
                      </div>
                    </div>
                  </td>
        ...
        {filteredCustomers.length === 0 && (
          <div className="py-20 text-center">
            ...
            <p className="text-[#8A8279] font-black uppercase tracking-widest text-[10px]">No Matching Clients Found</p>
          </div>
        )}
      </div>
            ...
            <div className="sticky top-0 z-10 bg-[#14141B]/95 backdrop-blur p-6 border-b border-[rgba(201,160,92,0.08)] flex items-start justify-between">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-[#F5F0E8] uppercase italic tracking-tight">
                ...
                <div className="mt-4 space-y-1 text-xs text-[#B8B0A4]">
                  <div className="flex items-center gap-2">
                    <Phone className="w-3.5 h-3.5 text-[#8A8279]" /> {selectedCustomer.contact_number || 'N/A'}
                  </div>
                  <div className="flex items-center gap-2">
                    <MapPin className="w-3.5 h-3.5 text-[#8A8279]" /> {selectedCustomer.address || 'No address'}
                  </div>
