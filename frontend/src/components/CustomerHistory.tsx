  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest text-[#8A8279]">Loading Customer Dashboard...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 text-left">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#14141B] rounded-2xl border border-[rgba(201,160,92,0.08)] p-5 space-y-2">
          ...
          <p className="text-2xl font-black text-[#F5F0E8]">{formatCurrency(summary.totalOutstanding)}</p>
          <p className="text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Outstanding ({summary.activeLoanCount} active)</p>
        </div>
        <div className="bg-[#14141B] rounded-2xl border border-[rgba(201,160,92,0.08)] p-5 space-y-2">
          <div className="w-10 h-10 rounded-xl bg-emerald-100 flex items-center justify-center">
            <TrendingUp className="w-5 h-5 text-emerald-600" />
          </div>
          <p className="text-2xl font-black text-[#F5F0E8]">{formatCurrency(summary.totalPaid)}</p>
          <p className="text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Total Paid ({summary.totalPayments} payments)</p>
        </div>
        <div className="bg-[#14141B] rounded-2xl border border-[rgba(201,160,92,0.08)] p-5 space-y-2">
          <div className="w-10 h-10 rounded-xl bg-amber-100 flex items-center justify-center">
            <Clock className="w-5 h-5 text-amber-600" />
          </div>
          <p className="text-2xl font-black text-[#F5F0E8]">{summary.totalLoans}</p>
          <p className="text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Total Loans</p>
        </div>
        <div className="bg-[#14141B] rounded-2xl border border-[rgba(201,160,92,0.08)] p-5 space-y-2">
          <div className="w-10 h-10 rounded-xl bg-rose-100 flex items-center justify-center">
            <AlertTriangle className="w-5 h-5 text-rose-600" />
          </div>
          <p className="text-2xl font-black text-[#F5F0E8]">{summary.overdueCount}</p>
          <p className="text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Overdue ({formatCurrency(summary.totalOverdue)})</p>
        </div>
      </div>

      <div className="bg-[#14141B] rounded-2xl border border-[rgba(201,160,92,0.08)] p-5 space-y-4">
        <div className="flex items-center justify-between">
          <p className="text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Loyalty Tier — Transaction Based</p>
          <span className={`px-2 py-0.5 rounded-full text-[9px] font-black text-white ${tierColors[data.tier] || 'bg-gray-600'}`}>
            {data.tier || 'Standard'}
          </span>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-2xl font-black text-[#F5F0E8]">{formatCurrency(data.transactionVolume ?? 0)}</p>
            <p className="text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Transaction Volume</p>
          </div>
          <div>
            <p className="text-2xl font-black text-[#F5F0E8]">{data.transactionCount ?? 0}</p>
            <p className="text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Transactions</p>
          </div>
        </div>
        {data.tierHistory && data.tierHistory.length > 0 && (
          <div className="space-y-2">
            <p className="text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Tier History</p>
            <div className="space-y-1.5">
              {data.tierHistory.slice(0, 5).map((h) => (
                <div key={h.id} className="flex items-center justify-between gap-3">
                  ...
                  <p className="text-[10px] font-semibold text-[#8A8279]">{formatDateTime(h.changedAt)}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {data.nextDuePayment && (
          <div className="bg-[#14141B] rounded-2xl border border-[rgba(201,160,92,0.08)] p-5 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-wider text-[#8A8279]">Next Payment Due</p>
              <p className="text-lg font-black text-[#F5F0E8]">
                {formatCurrency(data.nextDuePayment.amount)}
              </p>
              <p className="text-xs font-bold text-[#8A8279]">
        ...
        {data.recentActivity.length > 0 && (
          <div className="lg:col-span-2 bg-[#14141B] rounded-2xl border border-[rgba(201,160,92,0.08)] p-5">
            <p className="text-[10px] font-black uppercase tracking-wider text-[#8A8279] mb-3">Recent Activity</p>
            <div className="space-y-2">
              {data.recentActivity.slice(0, 5).map((a) => (
                <div key={a.id} className="flex items-center gap-3">
                  <ShieldCheck className="w-4 h-4 text-[#C9A05C] shrink-0" />
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-bold text-[#F5F0E8] truncate">{a.title}</p>
                    <p className="text-[10px] font-semibold text-[#8A8279]">{formatDateTime(a.createdAt)}</p>
                  </div>
                  {a.loanId && (
                    <button
