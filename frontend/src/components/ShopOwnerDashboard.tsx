    ...
    <div className="min-h-screen bg-[#FDFDFF] p-8 space-y-8">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <h1 className="text-4xl font-light text-[#EAE2D6]">
            Enterprise <span className="font-bold">Overview</span>
          </h1>
          <p className="text-[#6B655C] font-medium mt-1">Manage your branches, staff, and interest policies.</p>
        </div>

        <div className="flex gap-4">
          <button className="flex items-center gap-2 bg-[#14141B] border border-[rgba(201,160,92,0.12)] px-6 py-3 rounded-2xl font-bold text-[#6B655C] hover:bg-[#1C1C26] transition-all">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-[#14141B] p-6 rounded-[32px] border border-[rgba(201,160,92,0.08)] shadow-sm text-left">
          <p className="text-xs font-bold text-[#6B655C] uppercase tracking-widest mb-4">Current Policy</p>
          <div className="flex items-end gap-2">
            <h2 className="text-3xl font-bold text-[#EAE2D6]">3.5%</h2>
            <span className="text-[#6B655C] text-[10px] font-bold mb-1.5 uppercase">Monthly Interest</span>
          </div>
        </div>
        ...
        {config.hr_enabled ? (
          <div className="bg-slate-900 p-6 rounded-[32px] shadow-xl text-left animate-in slide-in-from-right-4 duration-500">
            <p className="text-xs font-bold text-[#6B655C] uppercase tracking-widest mb-4">Total Staff</p>
          ...
          <div className="bg-[#1C1C26] p-6 rounded-[32px] border border-dashed border-slate-300 flex items-center justify-center">
            <p className="text-[#6B655C] text-xs font-bold uppercase tracking-widest">HR Module Disabled</p>
          </div>
        )}
      </div>
            ...
            {branches.map((branch) => (
              <div key={branch.id} className="group bg-[#14141B] p-6 rounded-3xl border border-[rgba(201,160,92,0.08)] hover:border-[rgba(201,160,92,0.2)] hover:shadow-xl hover:shadow-slate-200/50 transition-all flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-[#1C1C26] rounded-2xl flex items-center justify-center text-[#6B655C] group-hover:bg-[#C9A05C]/8 group-hover:text-[#C9A05C] transition-colors">
                    <Building size={24} />
                  </div>
                ...
                <button className="p-3 bg-[#1C1C26] rounded-xl text-[#6B655C] hover:bg-[#C9A05C] hover:text-white transition-all">
                  <ArrowRight size={20} />
                </button>
              </div>
            ))}
          ...
          <div className="bg-[#14141B] p-8 rounded-[32px] border border-[rgba(201,160,92,0.08)] shadow-sm space-y-8">
            <div className="space-y-4">
              <div className="flex justify-between items-center text-left">
                <label className="text-sm font-bold text-[#6B655C]">Set Branch Interest Rate</label>
              ...
              />
              <div className="flex justify-between text-[10px] font-black text-[#6B655C] uppercase tracking-widest">
                <span>0.5% (Min)</span>
            ...
            <div className="pt-6 border-t border-slate-50 text-left">
              <p className="text-xs text-[#6B655C] leading-relaxed italic">
