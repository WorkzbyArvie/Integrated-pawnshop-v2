    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-[#14141B] rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-auto">
            <div className="flex items-center justify-between p-6 border-b border-[rgba(201,160,92,0.08)]">
              <div>
                <h2 className="text-xl font-black text-[#F5F0E8]">Set Location — {locationModal.branch.name}</h2>
                <p className="text-sm text-[#8A8279] mt-1">Click the map or search an address to pin the branch location.</p>
              </div>
              <button
              ...
              >
                <X className="w-5 h-5 text-[#8A8279]" />
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-[rgba(201,160,92,0.08)]">
              <button
                onClick={() => setLocationModal({ open: false, branch: null, lat: null, lng: null, address: '', saving: false })}
                className="px-6 py-3 bg-[#1C1C26] text-[#B8B0A4] rounded-xl text-sm font-bold hover:bg-[#222228] transition-colors"
              >
                Cancel
              </button>
              <button
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          {onBack && (
            <button
              onClick={onBack}
              className="p-3 bg-[#14141B] rounded-xl border border-[rgba(201,160,92,0.08)] hover:border-[rgba(201,160,92,0.2)] transition-all text-[#8A8279] hover:text-[#C9A05C]"
            >
              <ArrowLeft size={20} />
            </button>
          )}
          <div>
            <h1 className="text-3xl font-black text-[#F5F0E8] flex items-center gap-3 tracking-tight">
              ...
              <span className="text-[#8A8279] font-medium">Branches</span>
            </h1>
            <p className="text-[#8A8279] font-medium mt-1 ml-16">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-32 bg-[#14141B] rounded-[3rem] border border-[rgba(201,160,92,0.08)]">
          ...
          <p className="text-[10px] font-black text-[#8A8279] uppercase tracking-[0.3em]">Loading Branches...</p>
        </div>
      ) : branches.length === 0 ? (
        <div className="py-20 text-center bg-[#14141B] rounded-[3rem] border-2 border-dashed border-[rgba(201,160,92,0.12)]">
          <Building2 className="w-12 h-12 text-slate-300 mx-auto mb-4" />
          <p className="text-[#8A8279] font-bold text-[10px] uppercase tracking-widest">No branches registered for this tenant.</p>
          <p className="text-[#8A8279] font-medium text-xs mt-2">Branches are created by the tenant owner from their dashboard.</p>
        </div>
      ) : (
            ...
            <div key={branch.id} className="bg-[#14141B] border border-[rgba(201,160,92,0.12)] rounded-[2.5rem] p-8 hover:shadow-2xl hover:border-[rgba(201,160,92,0.2)] transition-all group relative overflow-hidden">
              <div className="flex justify-between items-start mb-6 relative z-10">
                <div className="w-14 h-14 bg-[#1C1C26] rounded-2xl flex items-center justify-center group-hover:bg-[#C9A05C] transition-colors">
                  <Building2 className="w-7 h-7 text-[#8A8279] group-hover:text-white" />
                </div>
              ...
              <div className="relative z-10">
                <h3 className="text-2xl font-black text-[#F5F0E8] group-hover:text-[#C9A05C] transition-colors">{branch.name}</h3>
                <p className="text-[#8A8279] flex items-center gap-1.5 text-sm mb-4 font-bold">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div className="bg-[#1C1C26] p-3 rounded-2xl border border-[rgba(201,160,92,0.08)]">
                    <p className="text-[9px] font-black text-[#8A8279] uppercase tracking-widest mb-1">Active Tickets</p>
                    <p className="font-bold text-[#F5F0E8] text-sm">{branch.active_tickets}</p>
                  </div>
                  <div className="bg-[#1C1C26] p-3 rounded-2xl border border-[rgba(201,160,92,0.08)]">
                    <p className="text-[9px] font-black text-[#8A8279] uppercase tracking-widest mb-1">Staff</p>
                    <p className="font-bold text-[#F5F0E8] text-sm">{branch.staff_count}</p>
                  </div>
                  <div className="bg-[#1C1C26] p-3 rounded-2xl border border-[rgba(201,160,92,0.08)]">
                    <p className="text-[9px] font-black text-[#8A8279] uppercase tracking-widest mb-1">Loan Value</p>
                    <p className="font-bold text-[#F5F0E8] text-sm">₱{branch.active_loan_value.toLocaleString()}</p>
                  </div>
                  <div className="bg-[#1C1C26] p-3 rounded-2xl border border-[rgba(201,160,92,0.08)]">
                    <p className="text-[9px] font-black text-[#8A8279] uppercase tracking-widest mb-1">Score</p>
                ...
                <div className="grid grid-cols-2 gap-4 py-4 border-y border-slate-50 mb-4">
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 bg-[#1C1C26] text-[#B8B0A4] rounded-xl group-hover:bg-[#C9A05C]/10 group-hover:text-[#C9A05C] transition-colors">
                      <Shield className="w-5 h-5" />
                    </div>
                    <div>
                      <p className="text-[10px] text-[#8A8279] uppercase font-black tracking-tighter">Manager</p>
                      <p className="text-sm font-bold text-[#F5F0E8]">{branch.manager_name || 'Unassigned'}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 bg-[#1C1C26] text-[#B8B0A4] rounded-xl group-hover:bg-[#C9A05C]/10 group-hover:text-[#C9A05C] transition-colors">
                    ...
                    <div>
                      <p className="text-[10px] text-[#8A8279] uppercase font-black tracking-tighter">At Risk</p>
                      <p className="text-sm font-bold text-[#F5F0E8]">{branch.at_risk_tickets} tickets</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 py-3 bg-[#1C1C26] text-[#8A8279] text-[10px] font-black uppercase rounded-xl group-hover:bg-blue-600/10 group-hover:text-blue-500 transition-all hover:shadow-md flex items-center justify-center gap-2"
                  >
